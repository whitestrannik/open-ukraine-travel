# 🏗 High-Level Design – Explore Ukraine Travel Portal

## 📌 Document Overview

This document outlines the high-level technical architecture and component design for the **Explore Ukraine Travel Portal**. It is aligned with the PRD and sets the foundation for clean, maintainable, and extensible implementation. All recommendations and decisions must follow industry-standard best practices.
...
- Build a mobile-first responsive web portal for international travelers that supports:
  - 📖 Browsing travel guidance content (static and dynamic)
  - 🛡 Purchasing tourist insurance
  - 🏨 Searching and booking hotels
  - 🗺 Exploring and reserving tours
...
|      Frontend (React)        |
+------------------------------+
|        API Gateway / BFF     |   ← Optional (can be skipped in P0)
+------------------------------+
|         Application Layer     |
+------------------------------+
|      Domain Logic Layer       |
...
|---------------------------|--------------------------------------------------------------------------------------------------|----------------|
| **Frontend**              | React SPA handling routing, UI rendering, and interactions with backend APIs                    | ✅ Yes         |
| **API Gateway / BFF**     | Proxy and request shaping between frontend and backend, useful if multiple clients exist         | ❌ No          |
| **Application Layer**     | Request-level logic, routing, orchestration of domain use cases                                  | ✅ Yes         |
| **Domain Logic Layer**    | Core business rules: availability checks, insurance calculations, content filters, etc.          | ✅ Yes         |
| **Infrastructure Layer**  | Adapters to database, email, payments, authentication, file storage, etc.                        | ✅ Yes         |
| **External APIs**         | 3rd-party systems (e.g. insurance providers, payment gateways) to be integrated via adapters     | ✅ Yes         |
...
| Insurance Integration | *(TBD – see integration spec)*    |
| Hotel Integration     | *(TBD – see integration spec)*         |
| Tour Integration      | *(TBD – see integration spec)*         |
